    ...
    val FlightAncillariesDetailsFinal = FlightAncillariesDetailsDf.get
      ...
      .agg(
        ...
        coalesce(sum(when($"business_model" === "CONSIGNMENT",$"total_fare__contract_currency__idr")
          ...
          lit(0)).as("gmv"),
        coalesce(sum($"commission_to_affiliate_idr" * -1),lit(0)).as("commission_to_affiliate"),
        coalesce(sum($"commission_idr" + $"total_fare___nta_idr"),lit(0)).as("commission"),
        coalesce(sum(
          when($"discount_premium_idr" < 0,$"discount_premium_idr" + $"discount_wht_idr")
        ),lit(0)).as("discount"),
        coalesce(sum($"premium_temp"),lit(0)).as("premium"),
