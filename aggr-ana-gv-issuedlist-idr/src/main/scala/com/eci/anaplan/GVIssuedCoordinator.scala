...
import com.eci.anaplan.aggregations.joiners.AnaplanGVIssuedlistIDR
import com.eci.anaplan.services.{GVIssuedDestination, GVIssuedStatusManager}
 ...
 */
class GVIssuedCoordinator @Inject()(spark: SparkSession,
                                    config: GVIssuedConfig,
                                    @Named("TENANT_ID") tenantId: String,
                                    anaplanDataframeJoiner: AnaplanGVIssuedlistIDR,
                                    statusManagerService: GVIssuedStatusManager,
                                    s3DestinationService: GVIssuedDestination,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Gift Voucher issuedlist in IDR from: $fromDate to: $toDate")
