...
import com.eci.anaplan.fa.summary.aggregations.joiners.FlightAncillariesSummary
import com.eci.anaplan.fa.summary.configs.Config
import com.eci.anaplan.fa.summary.services.{S3DestinationService, StatusManagerService}
...
class Coordinator @Inject()(spark: SparkSession,
                            ...
                            anaplanDataframeJoiner: FlightAncillariesSummary,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Flight Ancillaries Summary Aggregator from: $fromDate to: $toDate")
