package com.eci.anaplan.ci.details.services
...
class S3SourceService @Inject()(val sparkSession: SparkSession, config: Config,
  ...
  lazy val ConnectivityInternationalDf: DataFrame =
    readByDefaultCustom("connectivity_international.sales","booking_issue_date_date",true)
