package com.eci.anaplan.cd.idr

import com.eci.anaplan.cd.idr.aggregations.joiners.ConnectivityDomesticIDR
import com.eci.anaplan.cd.idr.configs.Config
import com.eci.anaplan.cd.idr.services.{S3DestinationService, StatusManagerService}
import com.eci.anaplan.cd.idr.services.StatusManagerService
...
class Coordinator @Inject()(spark: SparkSession,
                            ...
                            anaplanDataframeJoiner: ConnectivityDomesticIDR,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting  Connectivity Domestic in IDR Aggregator from: $fromDate to: $toDate")
