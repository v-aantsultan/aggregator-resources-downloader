...
import com.eci.anaplan.providers.{LPDetailsSlackClient, LPDetailsSparkSession}
 ...
 */
class LPDetailsModule extends AbstractModule {
  override def configure(): Unit = {
    ...
    bind(classOf[SparkSession]).toProvider(classOf[LPDetailsSparkSession]).asEagerSingleton()
    ...
    bind(classOf[SlackClient]).toProvider(classOf[LPDetailsSlackClient]).asEagerSingleton()
  }
}
