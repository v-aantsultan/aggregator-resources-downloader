...
import com.eci.anaplan.aggregations.joiners.AnaplanGVRedeemIDR
import com.eci.anaplan.services.{GVRedeemDestination, GVRedeemStatusManager}
 ...
 */
class GVRedeemCoordinator @Inject()(spark: SparkSession,
                                    config: GVRedeemConfig,
                                    @Named("TENANT_ID") tenantId: String,
                                    anaplanDataframeJoiner: AnaplanGVRedeemIDR,
                                    statusManagerService: GVRedeemStatusManager,
                                    s3DestinationService: GVRedeemDestination,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Gift Voucher Redeem in IDR Aggregator from: $fromDate to: $toDate")
