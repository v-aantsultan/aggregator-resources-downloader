package com.eci.anaplan.bs.details.services
...
class S3SourceService @Inject()(val sparkSession: SparkSession, config: Config,
  ...
  lazy val BusDf: DataFrame = readByDefaultCustom("bus.sales_v2","booking_issue_date_date",true)

  lazy val BusinessPartnerDf: DataFrame = readByDefaultCustomDtl("eci_sheets/ecidtpl_anaplan_fpna/Mapping Business Partner - Bus")
