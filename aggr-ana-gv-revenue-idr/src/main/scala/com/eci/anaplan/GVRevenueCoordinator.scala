...
import com.eci.anaplan.configs.GVRevenueConfig
import com.eci.anaplan.aggregations.joiners.AnaplanGVRevenueIDR
import com.eci.anaplan.services.{GVRevenueDestination, GVRevenueStatusManager}
 ...
 */
class GVRevenueCoordinator @Inject()(spark: SparkSession,
                                     config: GVRevenueConfig,
                                     @Named("TENANT_ID") tenantId: String,
                                     anaplanDataframeJoiner: AnaplanGVRevenueIDR,
                                     statusManagerService: GVRevenueStatusManager,
                                     s3DestinationService: GVRevenueDestination,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Gift Voucher Revenue in IDR Aggregator from: $fromDate to: $toDate")
