package com.eci.anaplan.cd.detail.services
...
class S3SourceService @Inject()(val sparkSession: SparkSession, config: Config,
  ...
  lazy val ConnectivityDomesticDf: DataFrame = readByDefaultCustom("ebill.bell_negative_revenue_detail","booking_issue_date_date",true)
