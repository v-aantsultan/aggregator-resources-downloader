...
import com.eci.anaplan.train.details.aggregations.joiners.GlobalTrainDetails
import com.eci.anaplan.train.details.configs.Config
import com.eci.anaplan.train.details.services.{S3DestinationService, StatusManagerService}
...
class Coordinator @Inject()(spark: SparkSession,
                            ...
                            anaplanDataframeJoiner: GlobalTrainDetails,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Global Train Details Aggregator from: $fromDate to: $toDate")
