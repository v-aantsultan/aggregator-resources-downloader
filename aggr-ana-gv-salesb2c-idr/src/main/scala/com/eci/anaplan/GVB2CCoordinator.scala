...
import com.eci.anaplan.services.{GVB2CDestination, GVB2CStatusManager}
 ...
 */
class GVB2CCoordinator @Inject()(spark: SparkSession,
                                 config: GVB2CConfig,
                                 @Named("TENANT_ID") tenantId: String,
                                 anaplanDataframeJoiner: AnaplanGVSalesB2CIDR,
                                 statusManagerService: GVB2CStatusManager,
                                 s3DestinationService: GVB2CDestination,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Gift Voucher Sales B2C in IDR Aggregator from: $fromDate to: $toDate")
