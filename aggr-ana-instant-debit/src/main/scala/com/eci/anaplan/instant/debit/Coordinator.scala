...
import com.eci.anaplan.instant.debit.configs.Config
import com.eci.anaplan.instant.debit.services.StatusManagerService
import com.eci.common.LoggerSupport
import com.eci.common.TimeUtils.{toTimestamp, utcDateTimeStringReport}
import com.eci.anaplan.instant.debit.services.S3DestinationService
...
class Coordinator @Inject()(spark: SparkSession,
                            ...
                            anaplanDataframeJoiner: InstantDebitJoiner,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Instant Debit Sales Aggregator from: $fromDate to: $toDate")
      ...
      case Success(anaplanDF) => {
        ...
        Try(s3DestinationService.publishToS3(anaplanDF, destination)) match {
          ...
          case Success(filePath) => {
            ...
            // Write to status DB
            Try(
              statusManagerService.markUnprocessed(
              applicationId,
