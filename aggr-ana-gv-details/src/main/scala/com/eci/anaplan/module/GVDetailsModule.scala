...
import com.eci.anaplan.providers._
 ...
 */
class GVDetailsModule extends AbstractModule {
  override def configure(): Unit = {
    ...
    bind(classOf[SparkSession]).toProvider(classOf[GVDetailsSparkSession]).asEagerSingleton()
    ...
    bind(classOf[SlackClient]).toProvider(classOf[GVDetailsSlackClient]).asEagerSingleton()
  }
}
