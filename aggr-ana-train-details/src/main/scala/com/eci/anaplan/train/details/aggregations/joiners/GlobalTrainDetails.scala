  ...
  private def joinDataFrames: DataFrame = {
    ...
    val GlobalTrainDetailsFinal = GlobalTrainDetailsDf.get
      ...
      .agg(
        ...
        coalesce(sum($"pax_quantity"),lit(0)).cast(IntegerType).as("transaction_volume"),
        coalesce(sum($"gmv_result"),lit(0)).cast(DecimalType(18,4)).as("gmv"),
        coalesce(sum($"gross_revenue_result"),lit(0)).cast(DecimalType(18,4)).as("gross_revenue"),
        ...
        coalesce(sum($"coupon_amount_idr"),lit(0)).cast(DecimalType(18,4)).as("coupon"),
        coalesce(sum($"nta_result"),lit(0)).cast(DecimalType(18,4)).as("nta"),
