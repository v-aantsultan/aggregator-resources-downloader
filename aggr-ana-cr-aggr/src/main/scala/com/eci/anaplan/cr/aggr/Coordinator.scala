...
import com.eci.anaplan.cr.aggr.aggregations.joiners.CouponReportAggr
import com.eci.anaplan.cr.aggr.configs.Config
import com.eci.anaplan.cr.aggr.services.{S3DestinationService, StatusManagerService}
import com.eci.anaplan.cr.aggr.services.StatusManagerService
...
class Coordinator @Inject()(spark: SparkSession,
                            ...
                            anaplanDataframeJoiner: CouponReportAggr,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting  Coupon Report in IDR Aggregator from: $fromDate to: $toDate")
