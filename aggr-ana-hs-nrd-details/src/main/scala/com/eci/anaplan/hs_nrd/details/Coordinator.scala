...
import com.eci.anaplan.hs_nrd.details.aggregations.joiners.HealthServiceDetails
import com.eci.anaplan.hs_nrd.details.configs.Config
import com.eci.anaplan.hs_nrd.details.services.{S3DestinationService, StatusManagerService}
...
class Coordinator @Inject()(spark: SparkSession,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Health Service by NRD in Details Aggregator from: $fromDate to: $toDate")
