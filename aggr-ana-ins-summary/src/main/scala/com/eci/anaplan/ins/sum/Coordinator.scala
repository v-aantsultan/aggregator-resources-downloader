...
import com.eci.anaplan.ins.sum.aggregations.joiners.InsuranceSummary
import com.eci.anaplan.ins.sum.configs.Config
import com.eci.anaplan.ins.sum.services.{S3DestinationService, StatusManagerService}
...
class Coordinator @Inject()(spark: SparkSession,
                            ...
                            anaplanDataframeJoiner: InsuranceSummary,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Insurance Summary Aggregator from: $fromDate to: $toDate")
