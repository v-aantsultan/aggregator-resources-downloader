package com.eci.anaplan.flight.details.services

import com.eci.anaplan.flight.details.configs.Config
...
class S3SourceService @Inject()(val sparkSession: SparkSession, config: Config,
  ...
  lazy val FlightDataFrame: DataFrame = readByDefaultCustomDtl("erp/flight_sales_revamp", "report_date");
  ...
  lazy val mappingAffiliateIdDf: DataFrame = readByDefaultCustomDtl("eci_sheets/ecidtpl_anaplan_fpna/Mapping Affiliate ID - Flight")
  lazy val mappingAirlineIdDf: DataFrame = readByDefaultCustomDtl("eci_sheets/ecidtpl_anaplan_fpna/Mapping Airline ID - Flight")
