package com.eci.anaplan.cd.detail

import com.eci.anaplan.cd.detail.aggregations.joiners.ConnectivityDomesticDetails
import com.eci.anaplan.cd.detail.configs.Config
import com.eci.anaplan.cd.detail.services.{S3DestinationService, StatusManagerService}
import com.eci.anaplan.cd.detail.services.StatusManagerService
...
class Coordinator @Inject()(spark: SparkSession,
                            ...
                            anaplanDataframeJoiner: ConnectivityDomesticDetails,
  ...
  def coordinate(): Unit = {
    ...
    slack.info(s"Starting Connectivity Domestic in Details Aggregator from: $fromDate to: $toDate")
