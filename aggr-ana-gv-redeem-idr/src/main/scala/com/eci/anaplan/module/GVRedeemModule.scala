...
import com.eci.anaplan.providers.{GVRedeemSlackClient, GVRedeemSparkSession}
 ...
 */
class GVRedeemModule extends AbstractModule {
  override def configure(): Unit = {
    ...
    bind(classOf[SparkSession]).toProvider(classOf[GVRedeemSparkSession]).asEagerSingleton()
    ...
    bind(classOf[SlackClient]).toProvider(classOf[GVRedeemSlackClient]).asEagerSingleton()
  }
}
